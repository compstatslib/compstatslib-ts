...
- **Parity with R, and a choice about it.** Each routine is checked against values R itself produces, withing a tolerance. Every routine that multiplies rounds each product once, through a software fused multiply-add, so a result is R's double bit for bit rather than Javascript's approximation. Where throughput matters more than exact precision, some routines taken and option for plain arithmetic instead, yielding over an order of performance speedup in exchange for very small changes in the last few decimal places of precision.
